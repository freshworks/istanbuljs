...
class VisitState {
  ...
  increase(type, id, index) {
    ...
    const existingAST = T.updateExpression('++', wrap(T.memberExpression(T.memberExpression(T.identifier(this.varName), T.identifier(type)), T.numericLiteral(id), true)), isPreIncrement);
    ...
    const trackTestExpression = T.callExpression(T.memberExpression(T.identifier('window'), T.identifier("trackTest")), [argsExpression]);
